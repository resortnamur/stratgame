...
class GraphicalGame:
    ...
    def maybe_spawn_scheduled_resources(self) -> list[str]:
        ...
        if self.turn in self.BONUS_5_SPAWN_TURNS:
            candidates = [
                terr for terr in self.territories
                if terr.owner >= 0
                and not self.is_onu_player(terr.owner)
                and not self.is_sanctuary_territory(terr.id)
            ...
            if candidates:
                ...
                messages.append(
                    f"Tour {self.turn}: une ressource +5 apparait sur {territory.name}. "
                    "Elle rapporte 5 renforts par tour et porte le plafond militaire de son controleur a 200."
                )

        if self.turn in self.PRECIOUS_MINERAL_MINE_SPAWN_TURNS:
            candidates = [
                terr for terr in self.territories
                if terr.owner >= 0
                and not self.is_onu_player(terr.owner)
                and not self.is_sanctuary_territory(terr.id)
                and terr.id not in self.precious_mineral_mine_ids
            ]
            if candidates:
                territory = random.choice(candidates)
                self.precious_mineral_mine_ids.add(territory.id)
                messages.append(
                    f"Tour {self.turn}: une mine de minerais precieux apparait sur {territory.name}. "
                    f"Elle rapporte {self.PRECIOUS_MINERAL_MINE_INCOME} ecus par tour a son controleur."
                )

        for message in messages:
            self.record_major_event(message)
        if messages:
            self.record_replay_snapshot(" | ".join(messages), force=True)
        return messages
    ...
    def get_ai_personality(self, player: int) -> str:
        if player not in self.ai_personalities:
            self.assign_ai_personality_to_player(player, "standard" if player in self.auto_controlled_players else None)
        return self.ai_personalities.get(player, "standard")
    ...
    def prepare_ai_behavior_for_turn(self, player: int) -> None:
        profile = self.get_ai_personality(player)
        if profile == "variable":
            self.ai_current_behavior[player] = random.choice(["standard", "aggressive", "defensive"])
        else:
            self.ai_current_behavior[player] = profile
    ...
    def spawn_pending_commercial_cities(self) -> List[str]:
        spawn_count = max(0, int(getattr(self, "pending_commercial_city_spawns", 0)))
        if spawn_count <= 0 or not self.territories:
            return []
        messages: List[str] = []
        for _ in range(spawn_count):
            forbidden = (
                set(self.sanctuary_territory_ids)
                | set(self.commercial_city_capital_ids.values())
                | set(getattr(self, "player_capital_ids", {}).values())
            )
            candidates = [
                terr for terr in self.territories
                if terr.id not in forbidden
                and terr.owner != self.onu_player_id
            ]
            if not candidates:
                candidates = [
                    terr for terr in self.territories
                    if terr.owner != self.onu_player_id and not self.is_any_capital_territory(terr.id)
                ]
            if not candidates:
                break
            territory = random.choice(candidates)
            ...
            self.commercial_city_players.add(new_player)
            self.commercial_city_capital_ids[new_player] = territory.id
            self.assign_ai_personality_to_player(new_player, "aggressive")
            ...
            # meme bonus et memes amenagements. Elle ne debarque pas avec un camion
            # de demenageurs pour effacer l'histoire locale, ce qui est presque civilise.
            self.sanctuary_territory_ids.discard(territory.id)
            ...
            message = f"Tour {self.turn}: nouvelle Cite commercante fondee sur {territory.name} sous le nom J{new_player + 1}."
            self.record_major_event(message)
            messages.append(message)
        self.pending_commercial_city_spawns = max(0, spawn_count - len(messages))
        self.refresh_last_stand_bonus_state()
    ...
    def maybe_spawn_random_bridge(self) -> Optional[str]:
        if random.randint(1, self.BRIDGE_SPAWN_DENOMINATOR) != 1:
            return None
        candidates = self.get_valid_bridge_candidates()
        if not candidates:
            return None
        ranked = sorted(
            candidates,
            key=lambda item: self.get_territory_graph_distance(item[0][0], item[0][1]),
            reverse=True,
        )
        preferred_count = max(1, math.ceil(len(ranked) / 4))
        key, points = random.choice(ranked[:preferred_count])
        self.add_bridge(key, points, fragile=True)
        a, b = key
        message = (
            f"Tour {self.turn}: un pont apparait entre {self.territories[a].name} "
            f"et {self.territories[b].name}."
        )
        self.record_major_event(message)
    ...
    def maybe_collapse_fragile_bridges(self) -> Optional[str]:
        bridges = sorted(
            set(getattr(self, "bridge_links", set()))
            & set(getattr(self, "fragile_bridge_links", set()))
        )
        if not bridges or random.randint(1, self.BRIDGE_COLLAPSE_DENOMINATOR) != 1:
            return None

        key = random.choice(bridges)
        a, b = key
        label = ""
        if 0 <= a < len(self.territories) and 0 <= b < len(self.territories):
            label = f"{self.territories[a].name}-{self.territories[b].name}"
        self.bridge_links.discard(key)
        self.fragile_bridge_links.discard(key)
        self.bridge_link_points.pop(key, None)
        self.recompute_neighbors_from_grid()

        message = f"Tour {self.turn}: un pont s'effondre"
        if label:
            message += f" ({label})"
        message += "."
        self.record_major_event(message)
    ...
    def integrate_due_submitted_territories(self, display: bool = True) -> List[str]:
        ...
        messages: List[str] = []
        for territory_id in sorted(list(getattr(self, "submitted_territory_ids", set()))):
            if not (0 <= territory_id < len(self.territories)):
                continue
            overlord = self.get_submitted_territory_overlord(territory_id)
            if overlord is None or overlord not in getattr(self, "nation_players", set()):
                # Le compteur reste valable uniquement tant que le suzerain est
                # toujours une nation, qu'elle soit humaine ou IA.
                self.submitted_territory_created_turns[territory_id] = self.turn
                continue
            created_turn = self.submitted_territory_created_turns.get(territory_id, self.turn)
            if self.turn - created_turn < self.SUBMITTED_TERRITORY_INTEGRATION_DELAY_TURNS:
                continue
            ...
            self.integrated_submitted_territories.setdefault(overlord, set()).add(territory_id)
            ...
            messages.append(
                f"Tour {self.turn}: {terr.name}, soumis depuis vingt tours consecutifs, "
    ...
    def calculate_submitted_territory_tribute(self, player: int) -> int:
        ...
        self.sanitize_submitted_territories()
        return sum(
            self.calculate_submitted_territory_income(self.territories[tid])
            for tid, overlord in self.submitted_territory_overlords.items()
            if overlord == player and 0 <= tid < len(self.territories)
        )

    def calculate_submitted_territory_income(self, territory: Territory) -> int:
        base_income = max(1, len(territory.neighbors))
        industrial_count = self.get_industrial_structure_count(territory.id)
    ...
    def should_submit_conquered_territory(self, attacker: int, territory: Territory, defeated_regiments: int = 1) -> bool:
        if getattr(self, "final_duel_active", False):
            return False
        if attacker not in getattr(self, "nation_players", set()):
            return False
        if self.is_sanctuary_territory(territory.id) or territory.owner == self.onu_player_id:
            return False
        if self.is_ai_player(attacker):
            return random.randint(1, self.AI_NATION_SUBMISSION_DENOMINATOR) == 1
    ...
    def submit_conquered_territory(self, territory_id: int, overlord: int, regiments: int) -> bool:
        if getattr(self, "final_duel_active", False):
            return False
        if overlord not in getattr(self, "nation_players", set()):
            return False
        if not (0 <= territory_id < len(self.territories)):
            return False
        terr = self.territories[territory_id]
        ...
        self.submitted_territory_ids.add(territory_id)
        self.submitted_territory_overlords[territory_id] = overlord
        self.submitted_territory_created_turns[territory_id] = self.turn
    ...
    def maybe_spawn_random_sanctuary_territory(self, display: bool = True) -> Optional[str]:
        ...
        Normal/chaos : 1 chance sur 10. Gouvernement mondial : 1 chance sur 7.
        """
        if random.randint(1, self.get_onu_spawn_denominator()) != 1:
            return None
        tax_haven_capitals = self.get_all_tax_haven_capital_ids()
        forbidden = (
            set(self.sanctuary_territory_ids)
            | set(self.ultra_super_territory_ids)
            | set(self.golden_territory_ids)
            | set(getattr(self, "precious_mineral_mine_ids", set()))
            | {terr.id for terr in self.territories if terr.reinforcement_bonus >= 3}
            | tax_haven_capitals
            | {terr.id for terr in self.territories if self.is_commercial_city_player(terr.owner)}
            | set(getattr(self, "player_capital_ids", {}).values())
        )
        candidates = [terr for terr in self.territories if terr.owner >= 0 and terr.id not in forbidden]
        if not candidates:
            return None
        ...
        self.refresh_eliminated_human_players()
        message = f"Tour {self.turn}: intervention de l'ONU. {terr.name} quitte J{previous_owner + 1} et devient territoire ONU fige."
        if display:
    ...
    def maybe_release_random_sanctuary_territory(self, display: bool = True) -> Optional[str]:
        ...
        Normal/chaos : 1 chance sur 15. Gouvernement mondial : 1 chance sur 10.
        """
        if random.randint(1, self.get_onu_release_denominator()) != 1:
            return None
        sanctuary_ids = [
            tid for tid in self.sanctuary_territory_ids
            if 0 <= tid < len(self.territories) and not self.is_submitted_territory(tid)
        ]
        if not sanctuary_ids:
            return None
        active_players = [player for player in self.get_active_players() if not self.is_commercial_city_player(player)]
        if not active_players:
            return None

        territory_id = random.choice(sanctuary_ids)
        terr = self.territories[territory_id]
        new_owner = random.choice(active_players)
        self.sanctuary_territory_ids.discard(territory_id)
        ...
        self.refresh_eliminated_human_players()
        message = f"Tour {self.turn}: retrait de l'ONU. {terr.name} devient un territoire ordinaire de J{new_owner + 1}."
        if display:
            self.show_message(message, 4600)
        return message

    def maybe_release_unstable_submitted_territories(self, display: bool = True) -> List[str]:
        """Chaque territoire soumis peut perdre son statut independamment.

        Contrairement aux territoires ONU ordinaires, il ne s'agit pas d'un tirage
        global qui libere au plus un territoire : chaque territoire soumis lance
        son propre 1 sur 40. Oui, les protectorats aussi ont decouvert les des.
        """
        self.sanitize_submitted_territories()
        ...
        for territory_id in submitted_ids:
            if random.randint(1, self.SUBMITTED_TERRITORY_INSTABILITY_DENOMINATOR) != 1:
                continue
            terr = self.territories[territory_id]
            previous_overlord = self.get_submitted_territory_overlord(territory_id)
            ai_recipients = [player for player in active_players if self.is_ai_player(player)]
            new_owner = random.choice(ai_recipients or active_players)
            ...
            if previous_overlord is not None:
                messages.append(
                    f"Tour {self.turn}: instabilite du territoire soumis. {terr.name} cesse d'etre soumis a J{previous_overlord + 1} "
                    f"et devient un territoire ordinaire de J{new_owner + 1}."
                )
            else:
                messages.append(
                    f"Tour {self.turn}: instabilite du territoire soumis. {terr.name} devient un territoire ordinaire de J{new_owner + 1}."
                )
    ...
    def add_industrial_structure(self, territory_id: int, structure_type: str) -> bool:
        structure_sets = self.get_industrial_structure_sets()
        target_set = structure_sets.get(structure_type)
        if target_set is None or not (0 <= territory_id < len(self.territories)):
            return False
        if territory_id in target_set:
            return False
        if self.get_industrial_structure_count(territory_id) > 0:
            return False
        target_set.add(territory_id)
        self.industry_territory_ids = set(self.factory_territory_ids)
        self.industrial_capture_counts.setdefault(territory_id, 0)
        self.industry_capture_counts = {tid: self.industrial_capture_counts.get(tid, 0) for tid in self.factory_territory_ids}
        return True

    def remove_all_industrial_structures(self, territory_id: int) -> int:
        removed = 0
        for ids in self.get_industrial_structure_sets().values():
            if territory_id in ids:
                ids.discard(territory_id)
                removed += 1
        self.industry_territory_ids = set(self.factory_territory_ids)
        self.industrial_capture_counts.pop(territory_id, None)
        self.industry_capture_counts = {tid: self.industrial_capture_counts.get(tid, 0) for tid in self.factory_territory_ids}
        return removed
    ...
    def add_cultural_center(self, territory_id: int, age: int = 0) -> bool:
        if not self.can_add_cultural_center(territory_id):
            return False
        self.cultural_center_ages.setdefault(territory_id, []).append(max(0, int(age)))
        self.cultural_capture_counts.setdefault(territory_id, 0)
        return True
    ...
    def add_university(self, territory_id: int) -> bool:
        if not self.can_add_university(territory_id):
            return False
        self.university_territory_ids.add(territory_id)
        self.university_capture_counts.setdefault(territory_id, 0)
        self.university_ages.setdefault(territory_id, 0)
        return True

    def remove_university(self, territory_id: int) -> bool:
        if territory_id not in self.university_territory_ids:
            return False
        self.university_territory_ids.discard(territory_id)
        self.university_capture_counts.pop(territory_id, None)
        self.university_ages.pop(territory_id, None)
        return True
    ...
    def add_temple(self, territory_id: int) -> bool:
        if not self.can_add_temple(territory_id):
            return False
        if not hasattr(self, "temple_territory_ids"):
            self.temple_territory_ids = set()
        if not hasattr(self, "temple_capture_counts"):
            self.temple_capture_counts = {}
        self.temple_territory_ids.add(territory_id)
        self.temple_capture_counts.setdefault(territory_id, 0)
        self.last_religion_foundation_message = None
        if 0 <= territory_id < len(self.territories):
            owner = self.territories[territory_id].owner
            if 0 <= owner < self.num_players:
                note = self.found_religion_if_possible(owner, territory_id)
                if note:
                    self.last_religion_foundation_message = note
                    self.record_major_event(f"Tour {self.turn}: {note}")
        return True

    def remove_temple(self, territory_id: int) -> bool:
        if territory_id not in getattr(self, "temple_territory_ids", set()):
            return False
        self.temple_territory_ids.discard(territory_id)
        self.temple_capture_counts.pop(territory_id, None)
        return True
    ...
    def build_wonder(self, territory_id: int, wonder_type: str, record_event: bool = True) -> bool:
        if wonder_type not in self.WONDER_DEFINITIONS:
            return False
        if wonder_type in getattr(self, "wonder_territories", {}):
            return False
        if not (0 <= territory_id < len(self.territories)):
            return False
        if self.get_wonder_type_at_territory(territory_id) is not None:
            return False
        territory = self.territories[territory_id]
        if territory.owner < 0 or self.is_onu_player(territory.owner):
            return False
        self.wonder_territories[wonder_type] = territory_id
        if wonder_type == "elyrion_sanctuary":
            religion_id = self.WONDER_RELIGION_ID
            self.religion_foundation_turns[religion_id] = self.turn
            self.religion_last_spread_turns[religion_id] = self.turn
            self.religion_holy_sites[religion_id] = territory_id
            self.apply_initial_religious_influence(religion_id, territory_id)
        elif wonder_type == "golden_pact_palace":
            self.enforce_commercial_city_wonder_exclusivity()
        if record_event:
            message = (
                f"Tour {self.turn}: J{territory.owner + 1} construit {self.get_wonder_name(wonder_type)} "
                f"sur {territory.name}. {self.get_wonder_effect(wonder_type)}."
            )
            self.record_major_event(message)
            self.record_replay_snapshot(message, force=True)
        return True
    ...
    def found_religion_if_possible(self, player: int, territory_id: int) -> Optional[str]:
        self.sanitize_religion_state()
        if player in self.religion_founders:
            return None
        used_religions = set(self.religion_founders.values())
        available = [religion_id for religion_id in range(self.WONDER_RELIGION_ID) if religion_id not in used_religions]
        if not available:
            return None
        religion_id = available[0]
        self.religion_founders[player] = religion_id
        self.religion_foundation_turns[religion_id] = self.turn
        self.religion_last_spread_turns[religion_id] = self.turn
        self.religion_holy_sites[religion_id] = territory_id
        self.apply_initial_religious_influence(religion_id, territory_id)
        name = self.get_religion_name(religion_id)
        symbol = self.get_religion_symbol(religion_id)
        return f"J{player + 1} fonde la religion {name} ({symbol}) sur {self.territories[territory_id].name}."
    ...
    def expand_religious_influences_if_due(self) -> List[str]:
        self.sanitize_religion_state()
        messages: List[str] = []
        for religion_id in sorted(getattr(self, "religion_foundation_turns", {})):
            founder = self.get_religion_founder(religion_id)
            if founder is None:
                continue
            temple_count = self.get_player_temple_count(founder)
            ...
            if interval is None:
                ...
                self.religion_last_spread_turns[religion_id] = self.turn
                continue
            founded_turn = self.religion_foundation_turns.get(religion_id, self.turn)
            last_spread_turn = self.religion_last_spread_turns.get(religion_id, founded_turn)
            if self.turn - last_spread_turn < interval:
                continue
            current_ids = [tid for tid, rid in self.religious_influence.items() if rid == religion_id]
            expansion_ids: set[int] = set()
            for tid in current_ids:
                if not (0 <= tid < len(self.territories)):
                    continue
                for neighbor_id in self.territories[tid].neighbors:
                    can_replace_existing_religion = (
                        religion_id == self.WONDER_RELIGION_ID
                        and self.religious_influence.get(neighbor_id) != religion_id
                    )
                    if (
                        0 <= neighbor_id < len(self.territories)
                        and (neighbor_id not in self.religious_influence or can_replace_existing_religion)
                        and not self.is_territory_tax_haven_immune_to_religion(neighbor_id)
                    ):
                        expansion_ids.add(neighbor_id)
            for tid in sorted(expansion_ids):
                if tid not in self.religious_influence or religion_id == self.WONDER_RELIGION_ID:
                    self.religious_influence[tid] = religion_id
            self.religion_last_spread_turns[religion_id] = self.turn
            if expansion_ids:
                messages.append(
                    f"{self.get_religion_name(religion_id)} etend son influence sur "
    ...
    def add_science_for_player(self, player: int) -> int:
        ...
        if science_income > 0:
            self.player_science[player] = self.get_base_player_science(player) + science_income
        else:
            self.player_science.setdefault(player, self.get_base_player_science(player))
    ...
    def calculate_player_culture(self, player: int) -> int:
        if player < 0 or self.is_onu_player(player):
            return 0
        culture = sum(self.calculate_territory_culture(terr) for terr in self.territories if terr.owner == player)
        if player in self.last_stand_bonus_players:
            culture *= 2
        if self.player_controls_wonder(player, "thousand_voices_theatre"):
            culture *= 2
        return culture
    ...
    def trigger_culture_expansions_if_due(self, player: int) -> tuple[list[str], int]:
        if player < 0 or self.is_onu_player(player):
            return [], 0
        if not hasattr(self, "culture_expansion_milestones"):
            self.culture_expansion_milestones = {}

        culture = self.calculate_player_culture(player)
        previous_milestone = max(0, int(self.culture_expansion_milestones.get(player, 0)) // 50 * 50)
        reached_milestone = max(0, int(culture) // 50 * 50)
        if reached_milestone <= previous_milestone:
            return [], culture

        # Tous les paliers deja franchis sont regroupes en une seule vague d'annexion.
        # Le palier maximal est consomme meme si aucun territoire voisin n'est disponible.
        self.culture_expansion_milestones[player] = reached_milestone
        target_ids = self.get_culture_expansion_target_ids(player)
        for territory_id in sorted(target_ids):
            self.annex_territory_by_culture(territory_id, player)
        ...
        self.refresh_destroyed_commercial_cities()
        self.refresh_eliminated_human_players()
        self.refresh_nation_states(trigger_player=player)

        # Une annexion peut apporter de nouveaux centres culturels. Les paliers ainsi
        # franchis pendant cette meme vague sont eux aussi consommes, sans seconde annexion.
        culture = self.calculate_player_culture(player)
        final_milestone = max(reached_milestone, max(0, int(culture) // 50 * 50))
        self.culture_expansion_milestones[player] = final_milestone

        captured_names = ", ".join(self.territories[tid].name for tid in sorted(target_ids)[:6])
        if len(target_ids) > 6:
            captured_names += ", ..."
        ...
        self.record_major_event(f"Tour {self.turn}: {message}")
        self.record_replay_snapshot(f"Tour {self.turn}: {message}", force=True)
    ...
    def has_culture_advantage(self, player: int, target_player: int, threshold: Optional[int] = None) -> bool:
        threshold = self.CULTURE_ADVANTAGE_THRESHOLD if threshold is None else threshold
        player_culture = self.calculate_player_culture(player)
        target_culture = self.calculate_player_culture(target_player)
    ...
    def activate_last_stand_bonus_if_needed(self, player: int) -> Optional[str]:
        ...
        if len(owned) != 1:
            ...
            return None
        terr = owned[0]
        was_active = player in self.last_stand_bonus_players and terr.id in self.get_player_tax_haven_capital_ids(player)
        self.add_tax_haven_capital(player, terr.id)
    ...
    def calculate_territory_income(self, territory: Territory) -> int:
        if territory.owner == self.onu_player_id:
            return 0
        base_income = max(1, len(territory.neighbors))
        industrial_count = self.get_industrial_structure_count(territory.id)
        income = base_income + (base_income * industrial_count)
        is_tax_haven_capital = territory.id in self.get_player_tax_haven_capital_ids(territory.owner)
        is_regular_capital = self.is_active_regular_capital(territory.id)
        if is_tax_haven_capital or is_regular_capital:
            income *= self.CAPITAL_INCOME_MULTIPLIER
        return income
    ...
    def calculate_player_income(self, player: int) -> int:
        if self.is_onu_player(player):
            return 0
        income = sum(self.calculate_territory_income(terr) for terr in self.territories if terr.owner == player)
        income += self.calculate_submitted_territory_tribute(player)
        if self.is_tax_haven_income_bonus_active(player):
            income = (income * 3) // 2
        if player in getattr(self, "nation_players", set()):
            income //= self.NATION_INCOME_DIVISOR
        income += self.get_religious_income_bonus(player)
        owned_mines = sum(
            1 for tid in self.precious_mineral_mine_ids
            if 0 <= tid < len(self.territories) and self.territories[tid].owner == player
        )
        income += owned_mines * self.PRECIOUS_MINERAL_MINE_INCOME
        return income
    ...
    def begin_player_turn(self, player: int) -> None:
        ...
        if self.is_human_player_id(player):
            pending_events = self.pending_major_events_for_humans.pop(player, [])
            if pending_events:
        ...
        destroyed_cc_notes = self.refresh_destroyed_commercial_cities()
        spawn_cc_notes = self.spawn_pending_commercial_cities() if player == min(self.get_active_players(), default=player) else []
        limit_note = self.enforce_last_stand_bonus_limits(begin_of_turn=True)
        mobilization_note = self.maybe_trigger_ai_mobilization(player)
        last_stand_note = self.activate_last_stand_bonus_if_needed(player)
        ai_alliance_note = self.maybe_trigger_random_ai_alliance(player)
        nation_notes = self.refresh_nation_states(trigger_player=player)
        colonization_notes = []
        income = self.collect_income_for_player(player)
        science_income = self.add_science_for_player(player)
        culture = self.calculate_player_culture(player)
        culture_expansion_notes, culture = self.trigger_culture_expansions_if_due(player)
        science = self.get_player_science(player)
        turn_notes = [*destroyed_cc_notes, *spawn_cc_notes, *nation_notes, *colonization_notes, *culture_expansion_notes, *[note for note in (limit_note, mobilization_note, last_stand_note, ai_alliance_note) if note]]
        if turn_notes:
            self.show_message(" ".join(turn_notes) + f" Revenu encaisse: +{income} ecu(s). Culture: {culture}. Science: +{science_income} (total {science}).", 4600)
        self.shop_action = None
        self.shop_mercenary_quantity = 1
        self.shop_gift_amount = 10
        ...
        self.geopolitical_panel_visible = False
        self.geopolitical_panel_page = 0
        self.record_replay_snapshot(f"Tour {self.turn} - debut du tour de J{player + 1}")
        self.reset_ai_turn_state()

    def register_special_capture(self, territory_id: int) -> List[str]:
        messages: List[str] = []
        terr_name = self.territories[territory_id].name
        if territory_id in self.fortress_territory_ids:
            count = self.fortress_capture_counts.get(territory_id, 0) + 1
            if count >= self.SPECIAL_CAPTURE_LIMIT:
                self.fortress_territory_ids.discard(territory_id)
                self.fortress_capture_counts.pop(territory_id, None)
                messages.append(f"Forteresse de {terr_name} detruite apres 3 captures.")
            else:
                self.fortress_capture_counts[territory_id] = count
                messages.append(f"Forteresse de {terr_name}: capture {count}/3.")

        if self.get_industrial_structure_count(territory_id) > 0:
            count = self.industrial_capture_counts.get(territory_id, 0) + 1
            if count >= self.SPECIAL_CAPTURE_LIMIT:
                removed = self.remove_all_industrial_structures(territory_id)
                ...
                self.industrial_capture_counts[territory_id] = count
                messages.append(f"Amenagements industriels de {terr_name}: capture {count}/3.")

        if territory_id in self.cultural_center_ages:
            count = self.cultural_capture_counts.get(territory_id, 0) + 1
            if count >= self.SPECIAL_CAPTURE_LIMIT:
                removed = len(self.cultural_center_ages.get(territory_id, []))
                self.cultural_center_ages.pop(territory_id, None)
                self.cultural_capture_counts.pop(territory_id, None)
                messages.append(f"{removed} centre(s) culturel(s) de {terr_name} detruit(s) apres 3 captures.")
            else:
                self.cultural_capture_counts[territory_id] = count
                messages.append(f"Centres culturels de {terr_name}: capture {count}/3.")

        if territory_id in self.university_territory_ids:
            count = self.university_capture_counts.get(territory_id, 0) + 1
            if count >= self.SPECIAL_CAPTURE_LIMIT:
                self.remove_university(territory_id)
                messages.append(f"Universite de {terr_name} detruite apres 3 captures.")
            else:
                self.university_capture_counts[territory_id] = count
                messages.append(f"Universite de {terr_name}: capture {count}/3.")

        if territory_id in getattr(self, "temple_territory_ids", set()):
            count = self.temple_capture_counts.get(territory_id, 0) + 1
            if count >= self.SPECIAL_CAPTURE_LIMIT:
                self.remove_temple(territory_id)
                messages.append(f"Temple de {terr_name} detruit apres 3 captures.")
            else:
                self.temple_capture_counts[territory_id] = count
    ...
    def maybe_trigger_market_event(self, display: bool = True) -> Optional[str]:
        ...
        total_lost = 0
        if event_type == "crash":
            for player in list(self.player_money):
                current = self.player_money[player]
                lost = (current * 2) // 3
                self.player_money[player] = current - lost
                total_lost += lost
            message = f"Tour {self.turn}: crash boursier. Tous les joueurs perdent deux tiers de leurs ecus economises ({total_lost} ecu(s) perdus)."
        else:
            for player in list(self.player_money):
                current = self.player_money[player]
                lost = current // 3
                self.player_money[player] = current - lost
                total_lost += lost
            message = f"Tour {self.turn}: crise boursiere. Tous les joueurs perdent un tiers de leurs ecus economises ({total_lost} ecu(s) perdus)."
        self.record_major_event(message)
        if display:
    ...
    def refresh_nation_states(self, trigger_player: Optional[int] = None) -> list[str]:
        if getattr(self, "final_duel_active", False):
            ...
            return []
        messages: list[str] = []
        if not hasattr(self, "nation_players"):
            self.nation_players = set()
        self.assign_all_ai_to_cold_war_camps()
        active_players = self.get_active_players()
        for player in list(self.nation_players):
            message = self.lose_nation_status_if_needed(player)
            if message:
                messages.append(message)
        for player in active_players:
            message = self.update_nation_qualification_progress(player)
            if message:
                messages.append(message)
        self.sanitize_nation_diplomacy()
        return messages
    ...
    def maybe_trigger_random_ai_alliance(self, player: int) -> Optional[str]:
        if not self.is_ai_player(player) or player not in getattr(self, "nation_players", set()):
            return None

        self.cleanup_expired_alliances()
        if random.randint(1, self.AI_NATION_ALLIANCE_DENOMINATOR) != 1:
            return None
        ...
        candidates = [
            ...
            if candidate != player
            and self.is_ai_player(candidate)
            and not self.is_onu_player(candidate)
            and not self.is_ai_alliance_active(player, candidate)
        ]

        # Le Palais du Pacte d'Or reserve la Cite commercante a son unique allie.
        if "golden_pact_palace" in getattr(self, "wonder_territories", {}):
            candidates = [candidate for candidate in candidates if not self.is_commercial_city_player(candidate)]

        if not candidates:
            return None

        ally = random.choice(candidates)
        key = self.normalize_ai_alliance_key(player, ally)
        expires_turn = self.turn + self.ALLIANCE_DURATION_TURNS
        self.active_ai_alliances[key] = expires_turn
        self.ai_alliance_start_turns[key] = self.turn
        message = (
    ...
    def cleanup_expired_alliances(self) -> None:
        if self.is_cold_war_active():
            ...
            self.assign_all_ai_to_cold_war_camps()
            return
        if self.active_alliances:
            self.active_alliances = {
                key: expires_turn
                for key, expires_turn in self.active_alliances.items()
                if self.turn < expires_turn
                and self.is_human_player_id(key[0])
                and self.is_ai_player(key[1])
                and any(terr.owner == key[0] for terr in self.territories)
                and any(terr.owner == key[1] for terr in self.territories)
            }
            self.alliance_start_turns = {key: start for key, start in self.alliance_start_turns.items() if key in self.active_alliances}
        if self.active_ai_alliances:
            self.active_ai_alliances = {
                key: expires_turn
                for key, expires_turn in self.active_ai_alliances.items()
                if self.turn < expires_turn
                and all(self.is_ai_player(player) for player in key)
                and all(not self.is_onu_player(player) for player in key)
                and all(any(terr.owner == player for terr in self.territories) for player in key)
            }
            self.ai_alliance_start_turns = {
                key: start_turn
                for key, start_turn in self.ai_alliance_start_turns.items()
                if key in self.active_ai_alliances
            }
        if self.active_offensive_alliances:
            self.active_offensive_alliances = {
                key: (target, expires_turn)
                for key, (target, expires_turn) in self.active_offensive_alliances.items()
                if self.turn < expires_turn
                and self.is_human_player_id(key[0])
                and self.is_ai_player(key[1])
                and any(terr.owner == key[0] for terr in self.territories)
                and any(terr.owner == key[1] for terr in self.territories)
                and any(terr.owner == target for terr in self.territories)
            }
    ...
    def maybe_trigger_sedition_at_end_of_turn(self) -> Optional[str]:
        ...
        for terr in self.territories:
            if terr.owner < 0 or self.is_sanctuary_territory(terr.id) or terr.owner == self.onu_player_id:
                continue
            if self.is_commercial_city_player(terr.owner):
                continue
            if terr.id in tax_haven_capitals:
                continue

            chance_points = self.calculate_sedition_chance_points(terr)
            if chance_points <= 0:
                continue
            if random.randint(1, self.SEDITION_DENOMINATOR) > chance_points:
                continue

            previous_owner = terr.owner
            new_owner = self.get_random_ai_recipient_for_sedition(previous_owner)
            terr.owner = new_owner
        ...
        message = f"Fin du tour {self.turn}: sedition. {len(seditions)} territoire{plural} change{plural} de camp ({details})."
        self.record_major_event(message)
    ...
    def is_attack_blocked_by_alliance(self, attacker: int, defender: int) -> bool:
        if attacker != defender and "golden_pact_palace" in getattr(self, "wonder_territories", {}):
            attacker_is_commercial = self.is_commercial_city_player(attacker)
            defender_is_commercial = self.is_commercial_city_player(defender)
            if attacker_is_commercial or defender_is_commercial:
                ally = self.get_commercial_city_wonder_ally()
                if ally is not None:
                    # Le Palais remplace entierement la diplomatie initiale de la CC :
                    # son controleur est protege, tous les autres joueurs restent des
                    # adversaires possibles dans les deux sens.
                    other_player = defender if attacker_is_commercial else attacker
                    return other_player == ally
        if attacker != defender and getattr(self, "final_duel_active", False):
            attacker_bloc = getattr(self, "final_duel_alliances", {}).get(attacker, attacker if self.final_duel_champions and attacker in self.final_duel_champions else None)
            defender_bloc = getattr(self, "final_duel_alliances", {}).get(defender, defender if self.final_duel_champions and defender in self.final_duel_champions else None)
            if attacker_bloc is not None and attacker_bloc == defender_bloc:
                return True
        if attacker != defender and self.is_cold_war_active():
            attacker_camp = self.get_cold_war_camp(attacker)
            defender_camp = self.get_cold_war_camp(defender)
            if attacker_camp is not None and attacker_camp == defender_camp:
                return True
        if attacker != defender:
            attacker_vassal_overlords = {
                overlord for tid, overlord in getattr(self, "vassal_territory_overlords", {}).items()
                if getattr(self, "vassal_players", {}).get(tid) == attacker
            }
            defender_vassal_overlords = {
                overlord for tid, overlord in getattr(self, "vassal_territory_overlords", {}).items()
                if getattr(self, "vassal_players", {}).get(tid) == defender
            }
            if defender in attacker_vassal_overlords or attacker in defender_vassal_overlords:
                return True
        if attacker != defender and self.is_nation_alliance_active(attacker, defender):
            if self.is_human_player_id(attacker) and self.is_ai_player(defender):
                return False
            return True
        if attacker != defender and self.is_commercial_city_player(attacker):
            if self.is_commercial_city_player(defender) or self.is_human_player_id(defender):
                return False
        if attacker != defender and self.is_ai_player(attacker) and self.is_ai_player(defender):
            if self.is_ai_alliance_active(attacker, defender):
                return True
            attacker_cc = self.is_commercial_city_player(attacker)
            defender_cc = self.is_commercial_city_player(defender)
            if attacker_cc != defender_cc:
                return True
        if self.is_ai_player(attacker) and self.is_human_player_id(defender):
            return self.is_alliance_active(defender, attacker)
        return False

    def break_alliance_due_to_human_attack(self, attacker: int, defender: int) -> Optional[str]:
        if not self.is_human_player_id(attacker) or not self.is_ai_player(defender):
            return None
        key = (attacker, defender)
        messages = []
        nation_message = None
        if self.is_nation_alliance_active(attacker, defender):
            nation_message = self.break_nation_alliance(attacker, defender, "attaque humaine")
            messages.append(f"alliance nationale rompue avec J{defender + 1}")
        if self.is_alliance_active(attacker, defender):
            self.active_alliances.pop(key, None)
            self.alliance_start_turns.pop(key, None)
            messages.append(f"alliance defensive rompue avec J{defender + 1}")
        if self.is_offensive_alliance_active(attacker, defender):
            self.active_offensive_alliances.pop(key, None)
            self.offensive_alliance_start_turns.pop(key, None)
    ...
    def execute_ai_economic_actions(self, player: int) -> int:
        if not self.is_ai_player(player) or self.is_onu_player(player):
            return 0
        if self.is_colonized_player(player):
            return len(self.execute_colonized_economic_actions(player))
        if self.is_commercial_city_player(player):
            return self.execute_commercial_city_economic_actions(player)
        self.ensure_player_economy(player)
        actions = 0

        # Priorite absolue des IA : construire les merveilles disponibles des que
        # le seuil de 50 science et le prix de 300 ecus sont atteints. Si l'argent
        # manque, l'IA epargne au lieu de le disperser dans des achats secondaires.
        while self.can_player_build_wonder(player) and self.get_available_wonder_types():
            wonder_action = self.find_ai_wonder_purchase(player)
            if wonder_action is None:
                break
            cost, callback = wonder_action
            if self.player_money[player] < cost:
                return actions
            self.player_money[player] -= cost
            callback()
            actions += 1

        if self.ai_needs_new_capital_as_nation(player):
            target = self.choose_ai_new_capital_target(player)
            if target is not None and self.player_money[player] >= self.CHANGE_CAPITAL_COST:
                self.player_money[player] -= self.CHANGE_CAPITAL_COST
                self.change_ai_capital_without_ui(player, target.id)
                message = f"Tour {self.turn}: J{player + 1}, nation sans capitale controlee, deplace sa capitale a {target.name}."
                self.record_major_event(message)
                actions += 1
            if actions > 0:
                self.refresh_nation_states(trigger_player=player)
                return actions
        while True:
            action = self.find_next_regular_ai_purchase(player)
            if action is None:
                break
    ...
    def execute_commercial_city_economic_actions(self, player: int) -> int:
        ...
        if self.ai_needs_new_capital_as_nation(player):
            ...
            if actions > 0:
                ...
                return actions
        while True:
            action = self.find_next_commercial_city_purchase(player)
            if action is None:
                break
            cost, callback = action
            if self.player_money[player] < cost:
    ...
    def grant_reinforcements(self, player: int) -> None:
        if self.is_onu_player(player):
            return
        if self.is_colonized_player(player):
            self.show_message(f"Joueur {player + 1}: aucun renfort tant que la colonisation dure.", 2200)
            return
        owned = [t for t in self.territories if t.owner == player]
        if not owned:
            return
        ...
        if total_regiments >= regiment_limit:
            self.show_message(
                f"Joueur {player + 1}: aucun renfort recu ({total_regiments} regiment(s), plafond {regiment_limit}). Mercenaires toujours achetables.",
                2600,
            )
            return
        ...
        base_reinforcements = min(self.MAX_REINFORCEMENTS_PER_TURN, math.ceil(controlled_territories / 3))
        bonus_reinforcements = ultra_owned * 3
        bonus_5_reinforcements = bonus_5_owned * 5
        religious_influenced_territories = self.get_national_religion_influenced_territory_count(player)
        religious_reinforcements = self.get_religious_reinforcement_bonus(player)
        ai_reinforcements = self.get_ai_reinforcement_bonus(player)
        reinforcements = base_reinforcements + bonus_reinforcements + bonus_5_reinforcements + religious_reinforcements + ai_reinforcements
        fortress_owned = [t for t in owned if t.id in self.fortress_territory_ids]
        ...
        if fortress_owned:
            ...
            for _ in range(priority_count):
                terr = random.choice(fortress_owned)
                if self.place_end_turn_reinforcement(terr, player):
                    university_conversions += 1
                fortress_reinforcements += 1

            for _ in range(reinforcements - priority_count):
                terr = random.choice(owned)
                if self.place_end_turn_reinforcement(terr, player):
                    university_conversions += 1
        else:
        ...
        self.show_message(
            f"Joueur {player + 1}: {reinforcements} renforts recus ({base_reinforcements} pour {controlled_territories} territoire(s) controle(s), arrondi superieur, plafond de base {self.MAX_REINFORCEMENTS_PER_TURN}{bonus_text}{bonus_5_text}{religion_text}{ai_bonus_text}; {ultra_owned} territoire(s) +3, {bonus_5_owned} territoire(s) +5{fortress_text}{university_text}).",
            2200,
        )
    ...
    def build_global_chaos_event_message(self, prefix: Optional[str] = None) -> Optional[str]:
        active_players = [player for player in self.get_active_players() if player not in getattr(self, "nation_players", set())]
        if not active_players:
            return None

        original_owned = {
            player: [terr for terr in self.territories if terr.owner == player]
            for player in active_players
        }
        ...
        for player in active_players:
            ...
            if lost_count <= 0:
                continue
            territories_to_lose = self.choose_owned_contiguous_block(player, lost_count)
        ...
        event_prefix = prefix if prefix is not None else f"Tour {self.turn}: episode de chaos."
        return (
    ...
    def maybe_trigger_chaos_event(self) -> bool:
        if self.difficulty_level != "chaos" or self.turn % 10 == 0:
            return False
    ...
    def maybe_trigger_empire_event(self) -> None:
        if self.last_empire_event_turn == self.turn:
            return

        self.last_empire_event_turn = self.turn
        self.selected_source = None
        self.selected_target = None
        self.turn_phase = "attack"

        turn_event_messages = []
        spawned_onu_message = self.maybe_spawn_random_sanctuary_territory(display=False)
        if spawned_onu_message:
            turn_event_messages.append(spawned_onu_message)
        submitted_integration_messages = self.integrate_due_submitted_territories(display=False)
        turn_event_messages.extend(submitted_integration_messages)
        submitted_instability_messages = self.maybe_release_unstable_submitted_territories(display=False)
        turn_event_messages.extend(submitted_instability_messages)
        vassal_integration_messages = self.integrate_due_vassals(display=False)
        turn_event_messages.extend(vassal_integration_messages)
        released_onu_message = self.maybe_release_random_sanctuary_territory(display=False)
        if released_onu_message:
            turn_event_messages.append(released_onu_message)
        # Les amenagements IA n'apparaissent plus gratuitement par evenement aleatoire.
        # Ils doivent provenir des achats IA explicites, des regles de depart ou des captures.
        if turn_event_messages:
            combined_event_message = " | ".join(turn_event_messages)
            self.record_major_event(combined_event_message)
            self.show_message(combined_event_message, 6200)

        if self.turn < 10:
            self.maybe_trigger_chaos_event()
            return
        ...
        if self.turn % 40 == 0:
            ...
            revolution_type = random.choice(["separate_rebels", "single_rebel_bloc"])
            split_count = 0
            ...
            for player in active_players:
                if player in getattr(self, "nation_players", set()) or self.is_commercial_city_player(player):
                ...
                lost_count = self.calculate_cultural_revolution_loss_count(player, len(owned))
                if lost_count <= 0:
                    skipped_players.append(player)
                    continue

                territories_to_split = self.choose_owned_contiguous_block(player, lost_count)
            ...
            if not pending_transfers:
                event_message = f"Tour {self.turn}: aucune revolution possible, tous les empires restants sont trop petits ou immunises."
                self.record_major_event(event_message)
                self.show_message(event_message, 3200)
                return
            ...
            event_message = f"Tour {self.turn}: {revolution_label}" + " | ".join(summary_parts) + skipped_text
            self.record_major_event(event_message)
            self.show_message(event_message, 6200)
            return
        ...
        active_players = [
            player
            for player in self.get_active_players()
            if player not in getattr(self, "nation_players", set())
            and not self.is_commercial_city_player(player)
        ]
        if not active_players:
            event_message = f"Tour {self.turn}: aucun evenement d'empire, aucun joueur actif non immunise."
            self.record_major_event(event_message)
            self.show_message(event_message, 3600)
            return

        territory_counts = {player: sum(1 for terr in self.territories if terr.owner == player) for player in active_players}
        max_count = max(territory_counts.values())
        leaders = [player for player, count in territory_counts.items() if count == max_count]
        target_player = random.choice(leaders)
        owned = [terr for terr in self.territories if terr.owner == target_player]

        default_lost_count = min(5, len(owned))
        lost_count = self.calculate_cultural_revolt_or_betrayal_loss_count(target_player, default_lost_count)
        if lost_count <= 0:
            event_message = f"Tour {self.turn}: evenement d'empire neutralise pour J{target_player + 1} ({self.get_culture_protection_label(target_player)})."
            self.record_major_event(event_message)
            self.show_message(event_message, 3600)
            return

        empire_event_index = self.turn // 10 - self.turn // 40
        is_betrayal = empire_event_index % 2 == 0
        territories_to_transfer = self.choose_owned_contiguous_block(target_player, lost_count)
        if not territories_to_transfer:
            event_message = f"Tour {self.turn}: evenement d'empire impossible pour J{target_player + 1}, aucune cible valide hors capitale."
            self.record_major_event(event_message)
            self.show_message(event_message, 3600)
            return
        lost_count = len(territories_to_transfer)
        ...
        if is_betrayal:
            eligible_receivers = [
                ...
                if player != target_player and not self.is_commercial_city_player(player)
            ]
            if not eligible_receivers:
                event_message = f"Tour {self.turn}: trahison impossible, aucun autre joueur ne peut recuperer les territoires de J{target_player + 1}."
                self.record_major_event(event_message)
                self.show_message(event_message, 3600)
                return
            ...
            beneficiary_player = random.choice(weakest_players)

            for terr in territories_to_transfer:
                terr.owner = beneficiary_player

            self.refresh_eliminated_human_players()
            event_message = f"Tour {self.turn}: trahison. J{target_player + 1} perd {lost_count} territoire(s) au profit de J{beneficiary_player + 1} ({self.get_culture_protection_label(target_player)})."
            self.record_major_event(event_message)
            self.show_message(event_message, 4600)
            return
        ...
        event_message = f"Tour {self.turn}: revolte chez J{target_player + 1}. {lost_count} territoire(s) rebelle(s) passent sous le controle de J{new_player + 1} ({self.get_culture_protection_label(target_player)})." + comeback_text
        self.record_major_event(event_message)
        self.show_message(event_message, 4600)

    def get_end_turn_move_limit(self, player: Optional[int] = None) -> int:
        player = self.current_player if player is None else player
        if self.count_player_territories(player) >= self.EXPANDED_END_TURN_MOVE_TERRITORY_THRESHOLD:
            return self.EXPANDED_END_TURN_MOVES
        return self.MAX_END_TURN_MOVES
    ...
    def maybe_trigger_ai_mobilization(self, player: int) -> Optional[str]:
        ...
        if player in getattr(self, "nation_players", set()):
            return None
        if random.randint(1, self.AI_MOBILIZATION_DENOMINATOR) != 1:
            return None
        ...
        frontier_candidates = self.get_ai_mobilization_frontier_territories(player)
        if not frontier_candidates:
            return None

        target = random.choice(frontier_candidates)
        total_regiments = sum(max(0, terr.regiments) for terr in owned)
    ...
    def complete_turn(self) -> None:
        ...
        self.execute_ai_economic_actions(previous_player)
        self.grant_reinforcements(previous_player)
        active_players = self.get_active_players()
        if active_players:
            ...
            if self.current_player <= previous_player:
                self.collecting_between_turn_events = True
                self.maybe_trigger_sedition_at_end_of_turn()
                winner = self.check_winner()
                if winner is not None:
                    self.collecting_between_turn_events = False
                    self.declare_victory(winner)
                    return
                self.turn += 1
                resource_messages = self.maybe_spawn_scheduled_resources()
                collapse_message = self.maybe_collapse_fragile_bridges()
                if collapse_message:
                    resource_messages.append(collapse_message)
                bridge_message = self.maybe_spawn_random_bridge()
                if bridge_message:
                    resource_messages.append(bridge_message)
                if resource_messages:
                    self.show_message(" | ".join(resource_messages), 6200)
                religion_notes = self.expand_religious_influences_if_due()
                if religion_notes:
                    self.record_major_event(f"Tour {self.turn}: " + " ".join(religion_notes))
                    self.show_message(" ".join(religion_notes), 5200)
                self.snapshot_tax_haven_turn_start_territory_counts()
                self.age_cultural_centers_one_turn()
                self.age_universities_one_turn()
                self.maybe_trigger_market_event()
                self.maybe_trigger_empire_event()
                active_players = self.get_active_players()
                sorted_players = sorted(active_players)
                if sorted_players:
                    self.current_player = sorted_players[0]
                self.collecting_between_turn_events = False
        self.selected_source = None
        self.selected_target = None
        if active_players:
    ...
    def check_winner(self) -> Optional[int]:
        self.last_victory_reason = ""
        owners = {t.owner for t in self.territories if t.owner >= 0}
        if not owners:
            return None
        if getattr(self, "final_duel_active", False) and getattr(self, "final_duel_champions", None):
            champions = list(self.final_duel_champions or [])
            alive_champions = [champion for champion in champions if any(t.owner == champion for t in self.territories)]
            eliminated_champions = [champion for champion in champions if champion not in alive_champions]
            if not eliminated_champions:
                return None

            pending_winner = getattr(self, "final_duel_pending_winner", None)
            if pending_winner in alive_champions:
                eliminated_label = ", ".join(f"J{champion + 1}" for champion in eliminated_champions)
                self.last_victory_reason = f"a provoque la disparition du champion {eliminated_label} dans la finale"
                return pending_winner

            if len(alive_champions) == 1:
                eliminated_label = ", ".join(f"J{champion + 1}" for champion in eliminated_champions)
                self.last_victory_reason = f"reste seul champion apres la disparition de {eliminated_label}"
                return alive_champions[0]

            def bloc_size(champion: int) -> int:
                return sum(
                    1 for terr in self.territories
                    if self.final_duel_alliances.get(terr.owner, terr.owner if terr.owner in champions else None) == champion
                )

            winner = max(alive_champions, key=lambda champion: (bloc_size(champion), -champion))
            eliminated_label = ", ".join(f"J{champion + 1}" for champion in eliminated_champions)
            self.last_victory_reason = f"gagne la finale apres la disparition de {eliminated_label}"
            return winner

        def accept_winner(owner: int, reason: str) -> Optional[int]:
            self.last_victory_reason = reason
            if self.maybe_start_final_duel(owner):
                return None
            return owner
        ...
        for owner in sorted(owners):
            owned_count = sum(1 for t in self.territories if t.owner == owner)
            if owned_count >= threshold:
                return accept_winner(owner, f"controle au moins les 3/4 des territoires ({owned_count}/{total_territories})")

        if len(self.golden_territory_ids) == 4:
            for owner in sorted(owners):
                if all(0 <= tid < len(self.territories) and self.territories[tid].owner == owner for tid in self.golden_territory_ids):
    ...
    def can_move_between(self, src: Territory, dst: Territory) -> bool:
        ...
        while stack:
            ...
            for neighbor_id in self.territories[territory_id].neighbors:
                neighbor = self.territories[neighbor_id]
                if neighbor.owner == self.current_player and neighbor_id not in visited:
                    stack.append(neighbor_id)
        return False
    ...
    def move_one_regiment(self, src: Territory, dst: Territory) -> bool:
        ...
        if self.turn_move_count >= move_limit:
            ...
            return False
        if src.owner != self.current_player or dst.owner != self.current_player:
            self.show_message("Le deplacement ne peut se faire qu'entre deux territoires allies.")
            return False
        if src.id == dst.id:
            self.show_message("Choisissez un territoire de destination different.")
            return False
        if src.regiments <= 1:
            self.show_message("Impossible : il doit rester au moins 1 regiment sur le territoire d'origine.")
            return False
        if not self.can_move_between(src, dst):
            self.show_message("Deplacement impossible : les deux territoires doivent etre relies par une continuite de territoires allies.")
            return False
        src.regiments -= 1
        dst.regiments += 1
        self.turn_move_count += 1
    ...
    def can_attack_specific_target(self, src: Territory, dst: Territory) -> bool:
        if self.is_colonized_player(self.current_player):
            return False
        if src.owner != self.current_player or dst.owner == self.current_player or dst.id not in src.neighbors or src.regiments < 2:
            return False
        if src.owner in getattr(self, "commercial_city_players", set()) and self.is_any_capital_territory(dst.id):
            return False
        if self.is_attack_blocked_by_alliance(src.owner, dst.owner):
            return False
        return True
    ...
    def resolve_attack_once(self, src: Territory, dst: Territory) -> Tuple[str, str, bool]:
        ...
        self.last_alliance_break_message = ""
        if src.owner in getattr(self, "commercial_city_players", set()) and self.is_any_capital_territory(dst.id):
            return "attaque interdite", "capitale protegee des Cites commercantes", False
        self.break_alliance_due_to_human_attack(src.owner, dst.owner)
        if src.regiments >= 5 and self.can_player_attack_with_four_dice(src.owner):
            att_dice = 4
        else:
            att_dice = 1 if src.regiments == 2 else 2 if src.regiments == 3 else 3
        if dst.id in self.fortress_territory_ids and dst.regiments >= 3:
            def_dice = 3
        else:
        ...
        if dst.regiments <= 0:
            ...
            was_sanctuary = self.is_sanctuary_territory(dst.id)
            was_submitted = self.is_submitted_territory(dst.id)
            attacker = src.owner
            submitted_regiments = max(1, min(def_losses, defender_regiments_before_loss))
            submit_instead_of_annex = (
                not was_sanctuary
                and not was_submitted
                and self.should_submit_conquered_territory(attacker, dst, submitted_regiments)
            )
            dst.owner = attacker
            ...
            culture_preserves_last_stand = (
                ...
                and attacker in self.last_stand_bonus_players
                and self.has_culture_advantage(attacker, previous_owner)
            )
            # Une soumission n'est pas une annexion : le territoire bascule en protectorat ONU
            # et ne doit donc jamais faire perdre le statut de paradis fiscal a l'attaquant.
            lost_last_stand_bonus = (
                False
                if submit_instead_of_annex or culture_preserves_last_stand
                else self.deactivate_last_stand_bonus_after_conquest(attacker)
            )
            special_capture_messages = self.register_special_capture(dst.id) if not submit_instead_of_annex else []
            original_capital_owner = self.get_regular_capital_owner(dst.id)
            ...
            if self.is_commercial_city_player(attacker):
                self.enforce_commercial_city_cultural_center_limit(attacker)

            if submit_instead_of_annex:
                forced_by_limit = self.should_force_submit_for_nation_limit(attacker)
                self.submit_conquered_territory(dst.id, attacker, submitted_regiments)
                if forced_by_limit:
                    special_capture_messages.insert(0, f"{dst.name} est automatiquement soumis par J{attacker + 1}: limite nationale atteinte ({self.get_nation_territory_limit(attacker)} territoires controles).")
                else:
            ...
            if was_sanctuary:
                self.sanctuary_territory_ids.discard(dst.id)
                self.submitted_territory_ids.discard(dst.id)
                self.submitted_territory_overlords.pop(dst.id, None)
                self.submitted_territory_created_turns.pop(dst.id, None)
                if self.is_human_player_id(attacker):
                    self.last_special_conquest_message = self.trigger_sanctuary_annexation_event(attacker)
                else:
                    self.last_special_conquest_message = f"Ordinateur J{attacker + 1}: annexion d'un sanctuaire ONU. Aucun effet special."
                if special_capture_messages:
                    self.last_special_conquest_message += " " + " ".join(special_capture_messages)
                nation_notes = self.refresh_nation_states(trigger_player=attacker)
                if nation_notes:
                    self.last_special_conquest_message += " " + " ".join(nation_notes)
                self.record_replay_snapshot(
                    f"Tour {self.turn} - J{attacker + 1} annexe le sanctuaire {dst.name}"
                )
                return f"{att_rolls}", f"{def_rolls}", True
            ...
            if previous_owner >= 0 and not any(t.owner == previous_owner for t in self.territories):
                if (
                    getattr(self, "final_duel_active", False)
                    and previous_owner in set(getattr(self, "final_duel_champions", ()) or ())
                ):
                    attacker_bloc = getattr(self, "final_duel_alliances", {}).get(
                        attacker,
                        attacker if attacker in set(getattr(self, "final_duel_champions", ()) or ()) else None,
                    )
                    if attacker_bloc is not None and attacker_bloc != previous_owner:
                        self.final_duel_pending_winner = attacker_bloc
                self.mark_eliminated_player_if_human(previous_owner)
                self.refresh_eliminated_human_players()
                winner = attacker
                owned = [t for t in self.territories if t.owner == winner]
                for _ in range(5):
                    if not owned:
                        break
                    terr = random.choice(owned)
                    terr.regiments += 1
                money_note = self.transfer_eliminated_player_money(previous_owner, winner)
                self.show_message(f"Le joueur {previous_owner + 1} est elimine ! Le joueur {winner + 1} recoit 5 regiments de bonus." + money_note, 3500)
            nation_notes = self.refresh_nation_states(trigger_player=attacker)
            if nation_notes:
                if self.last_special_conquest_message:
                    self.last_special_conquest_message += " " + " ".join(nation_notes)
            ...
            self.record_replay_snapshot(
                f"Tour {self.turn} - J{attacker + 1} {action} {dst.name}"
            )
            return f"{att_rolls}", f"{def_rolls}", True

        return f"{att_rolls}", f"{def_rolls}", False
